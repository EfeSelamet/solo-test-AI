...
def generate_child_boards(current_board):
    ...
    for from_idx, to_idx in possible_moves:
        ...
        #ceil is to get the real cost fo the parent node
        child_cost = ceil(current_board.cost) + heuristic(new_board)
...
def heuristic(child):
    return (len(legal_moves(child))) / 21 #(max moves possible)
