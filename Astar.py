...
def generate_child_boards(current_board):
    ...
    for from_idx, to_idx in possible_moves:
        ...
        child_cost = ceil(current_board.cost) + heuristic(new_board)  # Example cost function: increment by 1 per move
        
        children.append(treenodes(new_board, current_board,(from_idx, to_idx), child_cost))
    
    return children
#the cost is inverse with the number of possble moves
def heuristic(child):
    return (len(legal_moves(child)) + 1) / 13 #(max moves possible)
